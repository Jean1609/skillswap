    ...
    if request.method == "POST":
        ...
        if video and video.filename != "":
            if not allowed_file(video.filename):
                return "Formato inválido! Apenas vídeos são permitidos."
        ...
        # ganha 1 crédito ao criar curso
        cursor.execute("""
...
def ranking():
    ...
    SELECT usuarios.username,
           IFNULL(AVG(avaliacoes.nota), 0) as media
    FROM usuarios
    JOIN cursos ON usuarios.id = cursos.criador_id
    LEFT JOIN avaliacoes ON cursos.id = avaliacoes.curso_id
